...
from director_api.db.models import Job, ProjectCharacter
...
def _char_or_404(
    db: Session, tenant_id: str, project_id: UUID, character_id: UUID
) -> ProjectCharacter:
    c = db.get(ProjectCharacter, character_id)
    if not c or c.tenant_id != tenant_id or c.project_id != project_id:
...
def patch_character(
    ...
    c = _char_or_404(db, auth.tenant_id, project_id, character_id)
