...
test.describe("Studio smoke", () => {
  test("shell loads and shows Directely Studio", async ({ page }) => {
    await page.goto("/");
    await expect(page).toHaveTitle(/Directely/i);
    await expect(page.getByTestId("director-app-root")).toBeVisible({ timeout: 60_000 });
    await expect(page.getByRole("heading", { name: "Directely Studio" })).toBeVisible();
